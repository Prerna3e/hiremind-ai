...
const LoginPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || 'Authentication failed. Check your neural signature.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#050810', position: 'relative', overflow: 'hidden' }}>
            ...
            <motion.div
                ...
                className="glass"
                style={{ width: '100%', maxWidth: '480px', padding: '64px', borderRadius: '32px', position: 'relative', zIndex: 1, borderTop: '1px solid rgba(255,255,255,0.1)' }}
            >
                <div style={{ textAlign: 'center', marginBottom: '48px' }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '6px 14px', borderRadius: '100px', background: 'rgba(59, 123, 246, 0.1)', border: '1px solid rgba(59, 123, 246, 0.2)', marginBottom: '24px' }}>
                        <Sparkles size={12} color="var(--accent-blue)" />
                        <span style={{ fontSize: '10px', fontWeight: 800, color: 'var(--accent-blue)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Neural Access Protocol</span>
                    </div>
                    <h2 style={{ fontSize: '2.5rem', fontWeight: 900, letterSpacing: '-0.04em', marginBottom: '12px', color: 'white' }}>Welcome Back</h2>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '1rem' }}>Initiating secure session for HirMind <span style={{ color: 'var(--accent-cyan)' }}>AI</span></p>
                </div>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    ...
                    <button
                        ...
                        disabled={loading}
                        style={{ height: '64px', fontSize: '1.1rem', marginTop: '12px', boxShadow: 'var(--shadow-glow-blue)' }}
                    >
                    ...
                    <div style={{ textAlign: 'center' }}>
                        <Link to="/register" style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', textDecoration: 'none', fontWeight: 600 }}>
                            New to HireMind? <span style={{ color: 'var(--accent-cyan)' }}>Initialize account</span>
                        </Link>
                    </div>
                </form>
            </motion.div>
